...
. AWS 액세스 키를 입력한 경우 인스턴스의 위치를 선택하고 키 쌍을 선택하고 공용 IP 주소를 사용할지 여부를 선택한 다음 기존 IAM 역할을 선택하거나 필드를 비워 두면 Cloud Sync에서 역할을 생성합니다. KMS 키를 사용하여 데이터 브로커를 암호화할 수도 있습니다.
+
